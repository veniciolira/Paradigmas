...
def buscar_digitos(texto):
    digitos = [caractere for caractere in texto if caractere.isdigit()]
    if digitos:
        return int(digitos[0] + digitos[-1])
    return 0

soma_calibracao = sum(buscar_digitos(linha) for linha in calibration_data)

print(f"A soma total dos valores de calibração é: {soma_calibracao}")
